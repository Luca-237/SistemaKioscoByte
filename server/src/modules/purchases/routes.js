...
router.get('/', async (req, res, next) => {
    ...
    try {
        const compras = await new PurchasesService(req.tenantModels).list({ branchId: req.query.branchId });
...
router.post('/', async (req, res, next) => {
    ...
    try {
        ...
        const compra = await new PurchasesService(req.tenantModels).create(req.owner.clerkUserId, req.body);
