...
router.get('/', async (req, res, next) => {
    try {
        const branches = await Branch.find({ orgId: req.org._id, active: true }).sort({ name: 1 });
...
router.post('/', async (req, res, next) => {
    try {
        ...
        const branch = await Branch.create({ orgId: req.org._id, name: name.trim(), address, phone });
...
router.delete('/:id', async (req, res, next) => {
    try {
        ...
        await User.updateMany({ orgId: req.org._id }, { $pull: { branchIds: branch._id } });
