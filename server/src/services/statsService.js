...
class StatsService {
    ...
    async analytics({ branchId, from, to } = {}) {
        const match = {};
        if (branchId) match.branchId = oid(branchId);
        if (from || to) {
            match.createdAt = {};
        ...
        const [topArticles, purchaseHistory, saleHistory, operatorSales, monthlyBalance, expenseBreakdown, articleCatalog] = await Promise.all([
            ...
            this.models.Purchase.aggregate([
                { $match: match },
                { $unwind: '$items' },
                {
                    $group: {
            ...
            this.models.LedgerEntry.aggregate([
                { $match: match },
                {
                    $group: {
                        _id: {
            ...
            this.models.Article.find({ active: true }).select('_id name salePrice').lean()
        ]);
        ...
        const operatorNames = await this.models.User.find({}).select('_id name').lean();
