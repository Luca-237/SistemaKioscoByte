...
function operatorAuth(req, res, next) {
    ...
    try {
        ...
        req.operator = {
            userId: payload.sub,
            orgId: payload.orgId,
            adminId: payload.adminId, // Recovered from token
            branchId: payload.branchId,
            username: payload.username,
            name: payload.name
        };
        
        if (req.operator.adminId) {
            req.tenantModels = getTenantModels(req.operator.adminId);
        } else {
            return res.status(401).json({ success: false, message: 'Token inválido: falta adminId' });
        }
